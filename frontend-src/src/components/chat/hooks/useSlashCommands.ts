...
interface UseSlashCommandsOptions {
  ...
  onExecuteCommand: (command: SlashCommand, rawInput?: string) => boolean | void | Promise<boolean | void>;
}

const getCommandHistoryKey = (project: Project, provider: SessionProvider) => {
  const nodeSegment = project.nodeId || 'local';
  const projectSegment = project.fullPath || project.path || project.name;
  return `command_history_${provider}_${nodeSegment}_${projectSegment}`;
};
...
const readCommandHistory = (
  ...
  const history = safeLocalStorage.getItem(getCommandHistoryKey(project, provider));
  if (!history) {
    return {};
  }
...
const saveCommandHistory = (
  ...
  safeLocalStorage.setItem(getCommandHistoryKey(project, provider), JSON.stringify(history));
};
...
export function useSlashCommands({
  ...
  useEffect(() => {
    const fetchCommands = async () => {
      ...
      try {
        ...
        const parsedHistory = readCommandHistory(selectedProject, provider);
  ...
  const trackCommandUsage = useCallback(
    (command: SlashCommand) => {
      ...
      saveCommandHistory(selectedProject, provider, parsedHistory);
    },
    [provider, selectedProject],
  );
