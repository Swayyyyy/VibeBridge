...
const MAX_MENTIONABLE_FILES = 5000;
...
const flattenFileTree = (
  ...
  if (results.length >= MAX_MENTIONABLE_FILES) {
    return results;
  }
  ...
  files.forEach((file) => {
    if (results.length + flattened.length >= MAX_MENTIONABLE_FILES) {
      return;
    }

    const fullPath = basePath ? `${basePath}/${file.name}` : file.name;
    if (file.type === 'directory' && file.children) {
      flattened = flattenFileTree(file.children, fullPath, results.concat(flattened));
      return;
    }
  ...
  return results.concat(flattened).slice(0, MAX_MENTIONABLE_FILES);
};
...
export function useFileMentions({ selectedProject, input, setInput, textareaRef }: UseFileMentionsOptions) {
  ...
  const shouldLoadProjectFiles = useMemo(() => {
  ...
  useEffect(() => {
    ...
    if (!projectName || !shouldLoadProjectFiles || fileList.length > 0) {
      return;
    }
  ...
  }, [fileList.length, selectedProject?.name, shouldLoadProjectFiles]);
