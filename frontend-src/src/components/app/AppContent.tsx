...
export default function AppContent() {
  ...
  const routeProviderParam = searchParams.get('provider');
  const routeProvider =
    routeProviderParam === 'claude' || routeProviderParam === 'codex'
      ? routeProviderParam
      : undefined;
  const routeSessionId =
    searchParams.get('session') ||
    (sessionKey && !sessionKey.startsWith('draft-') ? sessionKey : undefined);
  const routeNodeId = searchParams.get('node') || undefined;
  const routeProjectName = searchParams.get('project') || undefined;
  const routeDraft =
    searchParams.get('draft') === '1' ||
    Boolean(sessionKey && sessionKey.startsWith('draft-'));
    ...
    <div className="fixed inset-0 flex bg-background">
      ...
      <div className={`flex min-w-0 flex-1 flex-col ${isMobile ? 'pb-mobile-nav' : ''}`}>
        <MainContent
          ...
          closeUtilityPanel={closeUtilityPanel}
          ws={ws}
